...
public class MapEntriesTest extends AbstractMappingMapEntriesTest {
    ...
    public void test_doNodeAdded1() throws Exception {
        ...
        addResource.invoke(mapEntries, new MapEntries.ChangeContext("/node", true, true), refreshed);
            ...
            Runnable r = () -> {
                try {
                    ...
                    for (int i1 = 0; i1 < 3; i1++) {
                        addResource.invoke(
                                mapEntries, new MapEntries.ChangeContext("/node", true, true), new AtomicBoolean());
                ...
                .thenAnswer((Answer<ServiceRegistration<ResourceChangeListener>>) invocation -> {
                    ...
                    Mockito.doAnswer(unregisterInvocation -> {
                                listener.onChange(List.of(
                                        new ResourceChange(ResourceChange.ChangeType.ADDED, "/resource", false)));
                                changeDelivered.set(true);
                                return null;
                            })
                            .when(registration)
