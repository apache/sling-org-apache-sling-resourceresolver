...
public class EtcMappingMapEntriesTest extends AbstractMappingMapEntriesTest {
    ...
    public void match_to_content_mapping() throws Exception {
        setupEtcMapResource(
                "test-node", http, PROP_REG_EXP, "localhost.8080/", PROP_REDIRECT_EXTERNAL, "/content/simple-match/");
    ...
    public void internal_to_external_node_mapping() throws Exception {
        ...
        expectedEtcMapping.assertEtcMap(
                "Etc Mapping for internal to external based on node", mapEntries.getResolveMaps());
    }
    ...
    public void nested_internal_mixed_mapping() throws Exception {
        Resource localhost = setupEtcMapResource(
                "localhost_any", http, PROP_REG_EXP, "localhost\\.\\d*", PROP_REDIRECT_INTERNAL, "/content");
        ...
        when(activator.getResourceResolverMetrics())
                .thenReturn(Optional.of(Mockito.mock(ResourceResolverMetrics.class)));
    ...
    //        expectedEtcMapping.assertEtcMap("Etc Mapping for regex map internal mapping",
    // mapEntries.getResolveMaps());
    //
    //        ResourceProvider<?> rp = new ResourceProvider<Object>() {
    //
    //            @Override
    //            public Resource getResource(ResolveContext<Object> ctx, String path, ResourceContext rCtx, Resource
    // parent) {
    //                return null;
    //            }
    //
